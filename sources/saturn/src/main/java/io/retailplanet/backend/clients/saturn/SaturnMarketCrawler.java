...
/**
 * MarketReader for "Saturn"
 *
 * @author w.glanzer, 12.01.2019
 */
@ApplicationScoped
public class SaturnMarketCrawler implements IMarketCrawler
{


  private static final Logger _LOGGER = LoggerFactory.getLogger(SaturnMarketCrawler.class);
  ...
  public List<CrawledMarket> getMarkets()
  {
    ...
    {
      _LOGGER.error("Failed to retrieve market list", e);
    }
  ...
  public void includeAvailability(@NotNull List<CrawledProduct> pProducts)
  {
    ...
    for (CrawledProduct product : pProducts)
    {
      executor.execute(() -> {
        ...
        {
          _LOGGER.error("Failed to retrieve availability for product " + product.getID());
        }
      });
    }
